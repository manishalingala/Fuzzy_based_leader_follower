...
#########################################################################################################
rospy.init_node('Po', anonymous=True) #Identify ROS Node
#######################################################################
#ROS Publisher Code for Velocity
pub1 = rospy.Publisher('robot2/ackermann_cmd', AckermannDriveStamped, queue_size=1)#Identify the publisher "pub1" to publish on topic "/turtle1/cmd_vel" to send message of type "Twist"
zizo = rospy.Publisher("/kalboz1", Float64, queue_size=10)
...
sub1 = rospy.Subscriber("robot2/odom", Odometry, callback_Init) 
def callback(data):
...
sub2 = rospy.Subscriber("robot2/odom", Odometry, callback) 

def callforward_Init(data):
  global pos_msg_0_3		#Identify msg variable created as global variable
  global sub1_3			#Identify a subscriber as global variable
  ...
  #rospy.loginfo(pos_msg_0)			#Print initial Pose of the vehicle on terminal
  sub1_3.unregister()				#Unsubsribe from this topic
  print("forward")
sub1_3 = rospy.Subscriber("/odom", Odometry, callforward_Init) #Identify the subscriber "sub1" to subscribe topic "/turtle1/pose" of type "Pose"
#######################################################################
#######################################################################
...
sub2_3 = rospy.Subscriber("/odom", Odometry, callforward) #Identify the subscriber "sub2" to subscribe topic "/turtle1/pose" of type "Pose"
#######################################################################
#######################################################################
